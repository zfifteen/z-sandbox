#!/usr/bin/env python3
"""
Benchmark script to find the highest bit size factorable in ≤10 seconds.
...
def test_bit_size(bit_size, num_tests=5, max_time=10.0):
...
def find_max_bit_size(start_bit=28, max_time=10.0):
    """Find the highest bit size with >0% success in max_time seconds."""
    print("BENCHMARK: Finding highest bit size factorable in ≤10 seconds")
    print("="*70)
    ...
    for bit_size in range(start_bit, 50):  # Up to 50 bits as safety limit
        ...
        if has_success:
            ...
            print(f"  → {bit_size}-bit: NO SUCCESS - Stopping search")
            break
