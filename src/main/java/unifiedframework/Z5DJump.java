 ...
 *
 * <p>This class reproduces the Z5D jump algorithm from the C demo_enhancements.c, implementing
 * intelligent candidate generation for prime search and factorization. Uses Z5D prime density
 * predictions to compute geodesic jump sizes instead of traditional incremental stepping (+1 or
 * +2).
 *
 * <p>Extended to use BigDecimal for ultra-high scales up to 10^1233.
 *
 * <p>Key features: - Geodesic jump calculation: jump_size = ln(prediction) * kappa_geo - Adaptive
 * jumping based on prime density - Deterministic output for reproducible results - Integration with
 * Z5dPredictor for prime location estimates - BigDecimal support for scales beyond double precision
 * limits
 */
public class Z5DJump {

  // Z Framework parameters (matching C demo constants)
  static final double ZF_Z5D_C_CALIBRATED = -0.00247;
  static final double ZF_KAPPA_STAR_DEFAULT = 0.04449;
  static final double ZF_KAPPA_GEO_DEFAULT = 0.3;
  ...
  public static double computeGeodesicJump(BigDecimal k) {
    ...
    String predStr =
        Z5dPredictor.z5dPrimeString(
            kStr, ZF_Z5D_C_CALIBRATED, ZF_KAPPA_STAR_DEFAULT, ZF_KAPPA_GEO_DEFAULT, true);
  ...
  /**
   * Parses ln(value) from a string in scientific notation. For "1.234e+123" returns ln(1.234) + 123
   * * ln(10)
   */
  static double parseLnFromScientificString(String str) {
  ...
  /**
   * Computes approximate ln(value) for BigDecimal values. Uses bit length of unscaled value for
   * accuracy.
   */
  static double approximateLn(BigDecimal bd) {
   ...
   */
  public static List<BigDecimal> generateJumpedCandidates(
      BigDecimal startK, int numCandidates, double baseIncrement) {
  ...
  public static void demonstrateJumping() {
    ...
    for (BigDecimal k : testKValues) {
      String predStr =
          Z5dPredictor.z5dPrimeString(
              k.toString(), ZF_Z5D_C_CALIBRATED, ZF_KAPPA_STAR_DEFAULT, ZF_KAPPA_GEO_DEFAULT, true);
      ...
      System.out.printf(
          "  k=%s: Z5D predicts prime ≈ %s%n",
          k.toEngineeringString(),
          predStr.substring(0, Math.min(100, predStr.length()))
              + (predStr.length() > 100 ? "..." : ""));
      if (Double.isFinite(lnPred)) {
        System.out.printf(
            "    Geodesic jump size: %.1f (ln(pred)=%.3f, kappa_geo=%.3f)%n",
            jumpSize, lnPred, ZF_KAPPA_GEO_DEFAULT);
      } else {
        System.out.printf(
            "    Geodesic jump size: %.1f (prediction invalid, using fallback)%n", jumpSize);
      }
    }
