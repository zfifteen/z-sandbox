 ...
 *
 * <p>This script demonstrates that the enhanced Z5D Predictor can now handle scales far beyond the
 * double precision limit (~10^305), reaching the target scale of 10^1233 as required for
 * cosmological and theoretical applications.
 */
  ...
  private static void demonstrateScaleRange() {
    ...
    for (int exp : exponents) {
      ...
      try {
        ...
        System.out.printf("%-10s %-20s %-15.2f %-10s%n", "10^" + exp, resultStr, elapsedMs, status);

      } catch (Exception e) {
        System.out.printf("%-10s %-20s %-15s %-10s%n", "10^" + exp, "ERROR", "-", "FAIL ✗");
      }
    }
