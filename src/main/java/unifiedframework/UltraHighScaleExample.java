...
/**
 * Simple example demonstrating the new BigDecimal API for ultra-high scale prime counting
 * predictions.
 *
 * <p>This example shows three ways to use the enhanced Z5D Predictor: 1. Traditional double
 * precision (for scales up to ~10^305) 2. BigDecimal API with BigDecimal objects 3. String-based
 * convenience API (easiest for ultra-high scales)
 */
public class UltraHighScaleExample {
  ...
  private static void example3_StringAPI() {
    ...
    for (String scale : scales) {
      ...
      System.out.printf(
          "  π(%s) ≈ %s%n", scale, result.substring(0, Math.min(30, result.length())) + "...");
    }
    System.out.println();
  }
  ...
  private static void example4_ExtendedResults() {
    ...
    if (status == Z5dPredictor.Z5D_SUCCESS) {
      ...
      System.out.printf("  Z5D Prediction:     %s%n", result.prediction.toEngineeringString());
      System.out.printf("  PNT Baseline:       %s%n", result.pntBase.toEngineeringString());
      System.out.printf("  Dilation term (d):  %s%n", result.dTerm.toEngineeringString());
      System.out.printf("  Curvature term (e): %s%n", result.eTerm.toEngineeringString());
      ...
      BigDecimal relDiff =
          result.prediction.subtract(result.pntBase).abs().divide(result.pntBase, mc);
