...
/** Unit tests for Z5DJump class. Verifies jump calculations and displays data for verification. */
public class Z5DJumpTest {
    ...
    for (int i = 0; i < testCases.length; i++) {
      ...
      String predStr =
          Z5dPredictor.z5dPrimeString(
              k.toString(),
              Z5DJump.ZF_Z5D_C_CALIBRATED,
              Z5DJump.ZF_KAPPA_STAR_DEFAULT,
              Z5DJump.ZF_KAPPA_GEO_DEFAULT,
              true);
      ...
      System.out.printf(
          "%-25s: jump_size = %.3f (ln(pred)=%.3f)%n", descriptions[i], jumpSize, lnPred);
      ...
      assertTrue(
          jumpSize >= 2.0, "Jump size must be at least fallback value for " + descriptions[i]);
    }
  ...
  public void testCandidateGeneration() {
    ...
    for (int i = 1; i < candidates.size(); i++) {
      assertTrue(
          candidates.get(i).compareTo(candidates.get(i - 1)) > 0,
  ...
  public void testJumpSizeScaling() {
    ...
    System.out.printf(
        "Ratio (k2/k1): %.1e%n", k2.divide(k1, java.math.MathContext.DECIMAL128).doubleValue());
