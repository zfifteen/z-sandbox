...
public class TestGVA {
    ...
    public void testSmallSemiprime() {
        ...
        assertTrue("Should find factor 3 or 5", factors.contains(BigInteger.valueOf(3)) || factors.contains(BigInteger.valueOf(5)));
    }

    @Test
    public void test64BitBenchmark() {
        // Generate a 64-bit semiprime
        BigInteger p = new BigInteger(32, java.util.Random.from(java.util.concurrent.ThreadLocalRandom.current()));
        BigInteger q = new BigInteger(32, java.util.Random.from(java.util.concurrent.ThreadLocalRandom.current()));
