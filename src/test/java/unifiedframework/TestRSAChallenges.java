...
public class TestRSAChallenges {
        ...
        for (RSAEntry e : entries) {
            if (!e.f1.isEmpty() && !e.f2.isEmpty()) {
                ...
                if (!prod.equals(N)) {
                    ...
                    System.err.printf("N mod 3=%d, p mod 3=%d, q mod 3=%d, (p*q) mod 3=%d%n",
                    ...
                    if (diff >= 0)
                        System.err.printf("first diff @ index %d: N[%d]=%c, prod[%d]=%c%n",
                ...
                Assertions.assertEquals(N, expectedP.multiply(expectedQ),
                        e.id + ": known factors don't multiply to N");
    ...
    void rsa260_neverSucceeds() throws Exception {
        RSAEntry e = entries.stream().filter(x -> "RSA-260".equals(x.id)).findFirst()
        ...
        List<BigInteger> adversarial = Arrays.asList(
                BigInteger.ONE, BigInteger.TWO, BigInteger.valueOf(3), BigInteger.valueOf(5),
                N, N.subtract(BigInteger.ONE), N.add(BigInteger.ONE)
        );
        List<BigInteger> candidates = sanitizeCandidates(N, adversarial);

        ExecutorService ex = Executors.newSingleThreadExecutor();
        Future<FactorizationShortcut.Factor> fut = ex.submit(() ->
                FactorizationShortcut.factorizeWithCandidatesBig(N, candidates, 64));
    ...
    private static final Map<String, Integer> EXPECTED_DIGITS = Map.of(
            "RSA-100", 100,
            "RSA-129", 129,
            "RSA-155", 155,
            "RSA-250", 250,
    ...
    static void assertSaneRSA(String id, BigInteger N) {
        ...
        for (int sp : small) {
            Assertions.assertNotEquals(0, N.mod(BigInteger.valueOf(sp)).intValue(),
