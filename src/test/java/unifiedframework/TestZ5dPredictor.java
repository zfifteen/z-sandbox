...
public class TestZ5dPredictor {
  ...
  public void testValidation() {
    ...
    System.out.printf(
        "z5dValidateK(100000.0): %d (expected: %d)%n", validK, Z5dPredictor.Z5D_SUCCESS);
    System.out.printf(
        "z5dValidateKappaGeo(0.3): %d (expected: %d)%n", validKappa, Z5dPredictor.Z5D_SUCCESS);
    ...
    System.out.printf(
        "z5dValidateK(1.0): %d (expected: %d)%n", invalidK1, Z5dPredictor.Z5D_ERROR_INVALID_K);
    System.out.printf(
        "z5dValidateK(-1.0): %d (expected: %d)%n", invalidK2, Z5dPredictor.Z5D_ERROR_INVALID_K);
    System.out.printf(
        "z5dValidateK(NaN): %d (expected: %d)%n", invalidK3, Z5dPredictor.Z5D_ERROR_DOMAIN);
    System.out.printf(
        "z5dValidateK(MAX_VALUE): %d (expected: %d)%n", invalidK4, Z5dPredictor.Z5D_ERROR_OVERFLOW);
    ...
    System.out.printf(
        "z5dValidateKappaGeo(-0.1): %d (expected: %d)%n",
        invalidKappa1, Z5dPredictor.Z5D_ERROR_INVALID_KAPPA_GEO);
    System.out.printf(
        "z5dValidateKappaGeo(20.0): %d (expected: %d)%n",
        invalidKappa2, Z5dPredictor.Z5D_ERROR_INVALID_KAPPA_GEO);
    System.out.printf(
        "z5dValidateKappaGeo(NaN): %d (expected: %d)%n",
        invalidKappa3, Z5dPredictor.Z5D_ERROR_DOMAIN);
  ...
  public void testCalibrationSelection() {
    ...
    System.out.printf(
        "Calibration for k=1000: c=%.5f, k*=%.5f, kappa=%.5f%n",
        cal1.c, cal1.kStar, cal1.kappaGeoFactor);
    System.out.printf(
        "Calibration for k=1e8: c=%.5f, k*=%.5f, kappa=%.5f%n",
        cal2.c, cal2.kStar, cal2.kappaGeoFactor);
    System.out.printf(
        "Calibration for k=1e11: c=%.5f, k*=%.5f, kappa=%.5f%n",
        cal3.c, cal3.kStar, cal3.kappaGeoFactor);
    ...
    System.out.printf(
        "All cal1 values finite: %b%n",
        Double.isFinite(cal1.c)
            && Double.isFinite(cal1.kStar)
            && Double.isFinite(cal1.kappaGeoFactor));
  ...
  public void testEdgeCases() {
    ...
    System.out.printf(
        "z5dBasePntPrime(100000.0): %.6f (finite: %b, positive: %b)%n",
        result, Double.isFinite(result), result > 0);
  ...
  public void testExtendedPrediction() {
    ...
    System.out.printf(
        "Result error code: %d (expected: %d)%n", result.errorCode, Z5dPredictor.Z5D_SUCCESS);
    System.out.printf(
        "Prediction: %.6f (finite: %b, positive: %b)%n",
        result.prediction, Double.isFinite(result.prediction), result.prediction > 0);
    System.out.printf(
        "PNT Base: %.6f (finite: %b, positive: %b)%n",
        result.pntBase, Double.isFinite(result.pntBase), result.pntBase > 0);
    System.out.printf(
        "D Term: %.6f (finite: %b, >=0: %b)%n",
        result.dTerm, Double.isFinite(result.dTerm), result.dTerm >= 0);
    System.out.printf(
        "E Term: %.6f (finite: %b, >=0: %b)%n",
        result.eTerm, Double.isFinite(result.eTerm), result.eTerm >= 0);
    System.out.printf(
        "Curvature Proxy: %.6f (finite: %b)%n",
        result.curvatureProxy, Double.isFinite(result.curvatureProxy));
    System.out.printf("C Used: %.6f (finite: %b)%n", result.cUsed, Double.isFinite(result.cUsed));
    System.out.printf(
        "K* Used: %.6f (finite: %b)%n", result.kStarUsed, Double.isFinite(result.kStarUsed));
    System.out.printf(
        "Kappa Geo Used: %.6f (finite: %b)%n",
        result.kappaGeoUsed, Double.isFinite(result.kappaGeoUsed));
  ...
  public void testAccuracyValidation() {
    ...
    int status =
        Z5dPredictor.z5dValidateAccuracy(
            kValues,
            truePrimes,
            kValues.length,
            -0.00247,
  ...
  public void testMathematicalConsistency() {
    ...
    double expectedPnt =
        k * (Math.log(k) + Math.log(Math.log(k)) - 1 + (Math.log(Math.log(k)) - 2) / Math.log(k));
  ...
  public void testScaleBehaviors() {
    ...
    for (double k : testKs) {
      ...
      System.out.printf(
          "k=%.1f: result=%.6f, finite=%b, positive=%b, >k=%b%n",
  ...
  public void testPredictionPerformance() {
    ...
    // CSV header
    String csvHeader =
        "Timestamp,Scale,Prediction_Number,Prediction_Value,Time_ns,Time_ms,Total_Time_ns,Total_Time_ms\n";
    ...
    try (java.io.FileWriter csvWriter = new java.io.FileWriter(csvFileName)) {
      ...
      for (double scale : scales) {
        ...
        for (int i = 0; i < predictionsPerScale; i++) {
          ...
          csvWriter.write(
              String.format(
                  ...
                  predictionTimeNs,
                  predictionTimeMs,
                  predictionEnd - scaleStartTime,
                  (predictionEnd - scaleStartTime) / 1_000_000.0));
        ...
        System.out.printf(
            "Scale %.0e: %d predictions in %.2f ms%n",
            scale, predictionsPerScale, scaleTotalTimeMs);
  ...
  public void testSmokeHighScales() {
    ...
    for (int exp = 19; exp <= 50; exp++) {
      ...
      double relDiff =
          (pnt != 0 && Double.isFinite(pnt))
              ? Math.abs((result - pnt) / pnt)
              : Double.POSITIVE_INFINITY;
      boolean closeToPnt = relDiff < 0.1; // within 10%
      System.out.printf(
          "Scale 10^%d: k=%.0e, result=%.2e, PNT=%.2e, finite=%b, positive=%b, >k=%b, rel_diff_to_PNT=%.4f%n",
    ...
    for (int exp = 1; exp <= 10000; exp++) { // Upper bound way beyond double limit
      try {
        ...
        if (Double.isFinite(result) && Double.isFinite(pnt)) {
          ...
          System.out.printf(
              "Result/PNT non-finite at 10^%d (k=%.2e, result=%.2e, PNT=%.2e)%n",
              exp, k, result, pnt);
          break;
        }
      } catch (Exception e) {
