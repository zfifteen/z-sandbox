...
def draw_intersection_mult(
    ...
    if show_candidates and n_approx > 0:
        ...
        candidates = range(
            max(1, int(sqrt_n) - candidate_range),
            int(sqrt_n) + candidate_range + 1
        )
        ...
        ax.scatter(candidate_x, candidate_y, c='r', s=100, 
                  marker='o', alpha=0.7, 
...
def draw_intersection_mult_advanced(
    ...
    if highlight_factors:
        ...
        ax.axvline(x=p_pos, color='green', linewidth=2, 
                  linestyle='--', alpha=0.7, label=f'Factor p={p}')
        ax.axvline(x=q_pos, color='orange', linewidth=2,
                  linestyle='--', alpha=0.7, label=f'Factor q={q}')
...
def demo_basic_examples():
    ...
    fig1 = draw_intersection_mult(
        a_digits_1, b_digits_1, n_approx_1,
    ...
    fig2 = draw_intersection_mult(
        a_digits_2, b_digits_2, n_approx_2,
    ...
    print(f"  Factors found: p={p3} in candidates={p3 in candidates_3}, "
...
def demo_rsa_scale():
    ...
    if p_found or q_found:
        ...
        print(f"  Note: Factors may need larger candidate pool")
